# -*- coding: utf-8 -*-
"""
Created on Fri Apr 28 15:15:33 2017
...
tim_soft_send_list = {'380683506754':['teamsoft.qa@gmail.com', 'a.markov@proximaresearch.com', 'pugachovk@gmail.com']}
...
def get_drug(drug_id):
    try:
        ...
        drug_out = drug.json()
        if drug_out['name']=='' and  drug_out['form']=='' and drug_out['dose']=='' and drug_out['pack']=='' and drug_out['numb']=='' and drug_out['make']=='' and drug_out['note']=='':
            drug_out_result = {'name':'Нет в АПИ', 'form':'Нет в АПИ', 'dose':'Нет в АПИ', 'pack':'Нет в АПИ', 'numb':'Нет в АПИ', 'make':'Нет в АПИ', 'note':'Нет в АПИ'}
        else:
...
def ordered_drug(drug_out):
    ordered_drug=drug_out['name'] + ' ' + drug_out['make'] + ', '+'Форма выпуска: '+drug_out['form']+', '+'Дозировка: '+drug_out['dose']+', '+'Номер в упаковке: '+ drug_out['numb'] + drug_out.get('note', 'Нет примечания')    
    return ordered_drug 
...
def new_deal_send_to_AMO(data_to_send, raw, order):
    ...
    deal_to_send['request']['leads']['add'][0]['custom_fields'][4]['values'][0]['value']=get_drug(raw['id'])['form'] + get_drug(raw['id'])['dose'] + get_drug(raw['id'])["pack"] + get_drug(raw['id'])['numb'] + get_drug(raw['id'])['make'] + get_drug(raw['id'])['note']
...
#отправляем почту тестировщикам Тим-Софта
                            if order['phone'] in tim_soft_send_list.keys():
                                for raw in order['data']:
                                    if send_mail_to_client(order, tim_soft_send_list, key = 'phone')=='successfully sent the mail':
                                        print('Почта отправилась на тестировщика Тим-Софт', '{} ({})'.format(get_shop(order['id_shop'])['name'], get_shop(order['id_shop'])['mark']))                                       
